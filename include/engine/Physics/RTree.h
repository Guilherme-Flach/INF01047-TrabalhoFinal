...
class RTree {
  ...
  public:
    ...
    /// \param nodes Nodes vector to which results will be written
...
class RTFileStream {
  ...
  public:
    ...
    template <typename TYPE> std::size_t Write(const TYPE &a_value) {
        ASSERT(m_file);
    ...
    template <typename TYPE> std::size_t Read(TYPE &a_value) {
        ASSERT(m_file);
        return fread((void *)&a_value, sizeof(a_value), 1, m_file);
    }

    template <typename TYPE> std::size_t ReadArray(TYPE *a_array, int a_count) {
        ASSERT(m_file);
